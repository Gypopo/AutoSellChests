    ...
    public void playEffect(Player p) {
        if (this == BOSS_BAR) {
            ...
            p.spigot().sendMessage(ChatMessageType.ACTION_BAR, new TextComponent(Lang.PLACED_SELL_CHESTS_ACTION_BAR.get()
                    ...
                    .replace("%limit%", p.hasPermission("autosellchests.maxchests.override") ? Lang.PLACED_SELL_CHESTS_ACTION_BAR_MAX.get() : String.valueOf(AutoSellChests.getInstance().getManager().getMaxSell(p)))));
        }
    }
